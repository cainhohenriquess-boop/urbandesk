...
export type ProjectGovernanceRouteProject = {
  id: string;
  code: string | null;
  name: string;
...
export type ProjectGovernanceRouteContext = {
  tenantId: string;
  role: string;
  userId: string | null;
  userName: string | null;
  userEmail: string | null;
  project: ProjectGovernanceRouteProject;
  compatibility: ProjectSchemaCompatibility;
};

export async function resolveProjectGovernanceContext(
  req: NextRequest,
  projectIdRaw: string,
  mode: AccessMode
): Promise<ProjectGovernanceRouteContext | { response: NextResponse }> {
  ...
  if (mode === "write" && !compatibility.governanceOpsSchemaReady) {
    return {
      response: NextResponse.json(
        {
          error:
            compatibility.governanceOpsNotice ??
            "Pendências, riscos e histórico técnico ainda dependem de migration complementar.",
          code: "project_governance_schema_pending",
        },
        { status: 503 }
      ),
    };
