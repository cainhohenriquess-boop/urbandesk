...
// ─────────────────────────────────────────────
// Painel Lateral de Engenharia (Formulário)
// ─────────────────────────────────────────────
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="space-y-1.5">
          ...
          <input 
            ...
            placeholder="Ex: Rede Pluvial Rua A" className="w-full text-sm bg-background border border-border rounded-md px-3 py-2 outline-none focus:border-brand-500"
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <input 
            ...
            placeholder="0.00" className="w-full text-sm bg-background border border-border rounded-md px-3 py-2 outline-none focus:border-brand-500"
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea 
            ...
            placeholder="Detalhes adicionais da operação..." className="w-full text-sm bg-background border border-border rounded-md px-3 py-2 outline-none focus:border-brand-500 resize-none"
          />
        </div>

        <div className="pt-4 border-t border-border flex gap-2">
          <button type="button" onClick={cancelPendingFeature} className="flex-1 py-2 rounded-md bg-muted text-foreground text-sm font-medium hover:bg-muted/80">
            Cancelar
          </button>
          <button type="submit" className="flex-1 py-2 rounded-md bg-brand-600 text-white text-sm font-bold hover:bg-brand-700 shadow-sm">
            Salvar Projeto
          </button>
        </div>
      </form>
  ...
  const geometriesGeoJson = useMemo(() => {
    return {
      ...
      features: geometryFeatures.map(f => {
        ...
        return {
          type: "Feature",
          geometry: { type: f.type === "line" ? "LineString" : "Polygon", coordinates: f.type === "line" ? coords : [coords] },
            ...
            <Marker key={feature.id} longitude={feature.coords[0].lng} latitude={feature.coords[0].lat} anchor="bottom">
              <div className="relative group flex flex-col items-center cursor-pointer">
                <div className="absolute -top-12 hidden group-hover:flex items-center whitespace-nowrap rounded-lg bg-card px-2.5 py-1.5 text-[11px] font-bold text-foreground shadow-xl border border-border flex-col">
                  ...
                  <span className="text-warning-500 text-[9px] flex items-center gap-1 mt-0.5"><span className="h-1 w-1 bg-warning-500 rounded-full animate-ping"/>Não Salvo no BD</span>
                </div>
